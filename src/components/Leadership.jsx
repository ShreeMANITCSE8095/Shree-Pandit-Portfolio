...
import { leadershipList, engagementHighlights } from '../data/portfolioData';
...
const LeadershipItem = ({ item, index }) => {
  ...
  return (
    <div className="relative grid grid-cols-1 md:grid-cols-2 items-center w-full mb-12 md:mb-16">
      <div className={`${isLeft ? 'md:pr-10 md:text-right md:order-1' : 'md:order-1'}`}>
        {isLeft && (
          <div
            data-aos="fade-right"
            className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-orange-500/30 hover:shadow-[0_15px_35px_rgba(255,107,0,0.12)] transition-all duration-500"
          >
  ...
  return (
    <section className="bg-[#0a0a0a] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]">
      ...
      <div className="max-w-6xl mx-auto relative z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-14">
          {engagementHighlights.map((item, index) => (
            <div
              key={item.title}
              data-aos="fade-up"
              data-aos-delay={index * 100}
              className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-5 md:p-6"
            >
              <p className="text-orange-300 text-[10px] font-black tracking-[0.35em] uppercase mb-2">
                Highlight
              </p>
